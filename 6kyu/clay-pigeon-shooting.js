...
function shoot(x){
  let firstShoot=x[0],
      secondShoot=x[1],
      thirdShoot=x[2],
      fistWinner=firstShoot[1]===true?Object.values(firstShoot[0]).join('').split('').filter((e)=>e!=' ').map((e)=>e==='X'?2:0):Object.values(firstShoot[0]).join('').split('').filter((e)=>e!=' ').map((e)=>e==='X'?1:0),
      P1first=fistWinner[0]+fistWinner[1],
      P2first=fistWinner[2]+fistWinner[3],
      
      secondWinner=secondShoot[1]===true?Object.values(secondShoot[0]).join('').split('').filter((e)=>e!=' ').map((e)=>e==='X'?2:0):Object.values(secondShoot[0]).join('').split('').filter((e)=>e!=' ').map((e)=>e==='X'?1:0),
      P1Second=secondWinner[0]+secondWinner[1],
      P2Second=secondWinner[2]+secondWinner[3],

      thirdWinner=thirdShoot[1]===true?Object.values(thirdShoot[0]).join('').split('').filter((e)=>e!=' ').map((e)=>e==='X'?2:0):Object.values(thirdShoot[0]).join('').split('').filter((e)=>e!=' ').map((e)=>e==='X'?1:0),
      P1Third=thirdWinner[0]+thirdWinner[1],
      P2Third=thirdWinner[2]+thirdWinner[3],

      P1Score=[P1first,P1Second,P1Third].reduce((acc,i)=>{return acc+=i}),
      P2Score=[P2first,P2Second,P2Third].reduce((acc,i)=>{return acc+=i})
